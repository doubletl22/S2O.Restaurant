...
export function BottomNavV2({ qrToken }: BottomNavV2Props) {
  ...
  // ✅ FIX chuẩn Next.js App Router
  const isActive = (path: string) => pathname.startsWith(path);

  return (
    <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] z-50">
      <div className="bg-white border-t h-16 flex items-center justify-around shadow-[0_-4px_10px_rgba(0,0,0,0.05)] px-2 pb-[env(safe-area-inset-bottom)]">
        ...
        <Link
          ...
          className={cn(
            ...
            isActive(baseUrl) && !isActive(`${baseUrl}/cart`) && !isActive(`${baseUrl}/tracking`)
              ? "text-orange-600"
              : "text-gray-400"
          )}
        >
