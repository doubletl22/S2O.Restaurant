...
/**
 * ✅ FIX: Chuẩn hóa data trả về từ hook/service (có thể là array hoặc Result{value})
 */
...
export default function BranchesPage() {
  ...
  const tablesQuery = useTables(selectedBranchId);

  const deleteBranchMutation = useDeleteBranch();
  const deleteTableMutation = useDeleteTable();

  // ✅ Ép kiểu luôn ra array để .map không bao giờ lỗi
  ...
  const handleDeleteTable = (table: any) => {
    if (confirm(`Xóa bàn "${table.name}"?`)) {
      deleteTableMutation.mutate(table.id, {
        onSuccess: () => toast.success("Đã xóa bàn"),
        onError: (err: any) => toast.error(err?.message || "Xóa bàn thất bại"),
      });
    }
  };
          ...
          <div className="flex flex-col gap-3">
            {isLoadingBranches ? (
              ...
              <div className="text-sm text-muted-foreground py-6 text-center">
                Chưa có chi nhánh nào. Hãy bấm <b>Thêm</b> để tạo chi nhánh.
              </div>
            ) : (
        ...
        <CardContent className="p-6 flex-1 overflow-auto bg-muted/10">
          {!selectedBranchId ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
              ...
              <Button variant="link" onClick={handleAddTable}>
                Thêm ngay
              </Button>
            </div>
          ) : (
